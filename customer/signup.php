...
if (
    ...
    if ($password != $cpassword) {
        ...
        if (isLengthOK($password)) {
            if (isStrongPassword($password)) { //desired task and continure coding for saving file and inserting user info
                if (move_uploaded_file($profile['tmp_name'], $profilePath)) {

                    $hascode = password_hash($password, PASSWORD_BCRYPT);
                    try {
                        $sql = "insert into users values
                            (?, ?, ?, ?, ?, ?, ?, ?)";
                        $stmt = $conn->prepare($sql);
                        $status = $stmt->execute([null, $email, $password, $profilePath, $city, $gender, $fullname, $bdate]);
                        if ($status) {
